...
#     with open(path) as fb:
#         reader=csv.reader(fb) #因为文件可能太多，借鉴生成器的使用来做
...
#                 # print(go)
#                 m.genre.add(go)#movie和genre有个manytomany的关系，在这里逐一添加关系，对应着就是在movie_genre表中添加数据，即添加关联关系
#             if i%1000==0:
#                 print(i)    # 控制台查看进度用
#             # pass
#
...
#             user_id_dct.setdefault(user_id,dict())#字典里加一个字典
#             user_id_dct[user_id][imdb_id]=rating #给这个字典赋值
#           #user_id_dct现在就长这样：{ user:{movie1:rating, movie2:rating, ...}, ...}
#     # 对所有用户和评分记录
...
#             relation=Movie_rating(user=u,movie=movie,score=rating,comment='')#在movie_rating表中添加关联关系
#             relation.save()
#             # break
...
'''恢复评分信息用，如果movie_rating表没有出错，不需要执行下面的函数，忽略'''
# def get_ratings():
...
# #导入电影相似度 movie_similarity.csv中的数据是根据hive+spark以及协同过滤算法得到的
...
class IndexView(ListView):  # 继承ListView类，后面context相关的操作会封装的很好用
    model = Movie  # 模型
    template_name = 'movie/index.html'  # 前端渲染的页面
    paginate_by = 15   # 一页展示15条数据
    context_object_name = 'movies'  # 列表模型在模板（前端页面）中的参数名称，方便在页面上进行遍历 for movie in movies
    ordering = 'imdb_id'  # 结果根据‘imbd_id'排序
    page_kwarg = 'p'  # 获取第几页的数据的参数名称。默认是`page`，例如#http://127.0.0.1:8000/article/?p=2 里的’p‘

    # 返回前1000部电影
    # `get_queryset`：如果你提取数据的时候，并不是要把所有数据都返回，那么你可以重写这个方法。将一些不需要展示的数据给过滤掉。
    def get_queryset(self):
        # filter条件查询，objects.filter(属性值1=值1，属性2=值2），逗号是and查询
        # __lte 查询谓词 使实现更灵活的查询
        # 前面双下划线 如__exact就是等值匹配
        # 这里的lte是小于等于的意思 lt小于  gt大于 gte大于等于
        return Movie.objects.filter(imdb_id__lte=1000)

    # 得到context数据
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super(IndexView, self).get_context_data(*kwargs)  # 就这样就可以得到context，父类提供的

        paginator = context.get('paginator')  # 页码
        page_obj = context.get('page_obj')  # 与页码相关的对象
    ...
    # 得到当前页和前后页的数据
    # around_count是指当前页面左右两边显示的页面数目，例如around_count=2，当前页面为4，则会显示2，3，4，5，6页面，其余用...代替
    # 前端页面会根据得到的数据确定需要显示出的页面页码
...
class PopularMovieView(ListView):
    ...
    def get_queryset(self):
        ...
        # 一个bug!这里filter出来虽然是正确的100部电影，但是会按照imdb_id排序，也就是得不到100部热门电影的正确顺序
        # movies=Movie.objects.filter(id__in=hot_movies.values("imdb_id"))

        # 要手动排序一次：
        # 为此查询 in 查找数据限定在指定范围内
        # annotate(名=聚合函数(列))
        movies=Movie.objects.filter(id__in=hot_movies).annotate(nums=Max('movie_hot__rating_number')).order_by('-nums')#orderby默认升序，要加个负号使降序
        # 现在就得到了真正100部热门电影了
        # movies.query可以得到这条查询语句的sql表示，其实就是把这条sql发给mysql
        return movies
...
class TagView(ListView):
    ...
    def get_queryset(self):
        ...
        if 'genre' not in self.request.GET.dict().keys():
            ...
            return movies[100:200]  # 就随便展示100-200吧
        else:
            # 如果在的话就找到并展示前100条
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            for error in errors:
                messages.info(request, error)  # 展示错误信息
...
class LoginView(View):
    # get返回页面
    def get(self, request):
        return render(request, 'movie/login.html')  # 默认template文件夹下
    # post处理数据
    def post(self, request):
        print(request.POST)
...
def UserLogout(request):
    # 登出，立即停止会话
    request.session.set_expiry(-1)  # 设置session失效时间，-1使立马失效
    return redirect(reverse('movie:index'))
...
class MovieDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 判断是否登录用
        login = True
        try:  # session封装好了，直接像字典一样用即可
            user_id = self.request.session['user_id']
        except KeyError as e:
            login = False  # 未登录

        # 获得电影的pk  pk就代表主键id  pk=12 相当于 id__exact=12
        pk = self.kwargs['pk']
...
class RecommendMovieView(ListView):
    ...
    def get_user_sim(self):
        ...
        # get方法，返回满足条件的唯一一条数据，如果没有数据或不止一条数据还会报错
        cur_user = User.objects.get(pk=cur_user_id)
        # 获取其它用户
        # exclude条件查询，不包含此条件的数据
        other_users = User.objects.exclude(pk=cur_user_id)

        self.cur_user_movie_qs = Movie.objects.filter(user=cur_user)  # 得到这个用户评论过的电影

        # 计算当前用户与其他用户评分过的电影交集数
        for user in other_users:
            # 记录感兴趣的数量
            # 计算其他用户评论过的电影与刚才得到的集合的交集，数目多就代表相似性高
    ...
    def get_recommend_movie(self, user_lst):
        ...
        # 用户，相似度
        for user, _ in user_lst:  # 遍历与当前用户相似的各个用户
            ...
            for movie in movie_set:  # 拿到这些用户评分过的电影，以每个电影作为键，评分记录作为值（累加），按值从大到小进行排序
